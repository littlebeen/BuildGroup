...
def is_exist(all_cloud,new_arr):
    for arr in all_cloud:
        ...
        for aug_time in range(aug_times + 1):
            ...
            for blockNum, block in enumerate(blocks):
                if (len(block) > 10000):
                    ...
                    # instance_ids=np.unique(block[:,-1]) #当时想看一下有没有很小的实例
                    ...
                    if split != 'test':
                        ...
                        if (split == 'train' or 'val' in split) and (
                            ...
                            print(len(coords))
                            torch.save((coords, colors, sem_labels, instance_labels), out_file_path)
                            # x = np.unique(instance_labels)
                            # print(len(coords)/len(x))
                            # n.append(len(coords)/len(x))
...
if __name__ == '__main__':
    city = 'Wuhu'
    data_folder = '../Urbanbis/'+city
    save_folder = '../Urbanbis/'+city+'_softgroup_building'
