...
def splitPointCloud(cloud, size=50.0, stride=50):
    limitMax = np.amax(cloud[:, 0:3], axis=0)  #xyz最大值
    width = int(np.ceil((limitMax[0] - size) / stride)) + 1  #只在xy轴上剪裁
...
def preparePthFiles(files, split, outPutFolder, AugTimes=0, crop_size=50):
    ...
    for file in files:
        ...
        for AugTime in range(AugTimes + 1):
            ...
            if split != 'test':
                coordShift['globalShift'] = list(points[:, :3].min(0)) #取xyz 求xyz三轴的最小值
            points[:, :3] = points[:, :3] - points[:, :3].min(0) #重新规划xyz

            blocks = splitPointCloud(points, size=crop_size, stride=crop_size)
            for blockNum, block in enumerate(blocks):
                if (len(block) > 10000):
                    outFilePath = os.path.join(outPutFolder,
                                               name + str(blockNum) + '_inst_nostuff.pth') #pth txt
                    if (block[:, 2].max(0) - block[:, 2].min(0) < zThreshold):  #z不够高的,说明地面上没啥东西？
                        block = np.append(
                            block, [[
                    ...
                    if split != 'test':
                        ...
                        coordShift[outFileName] = list(block[:, :3].mean(0))  #存一下切割后每张图的均值
                    coords = np.ascontiguousarray(block[:, :3] - block[:, :3].mean(0))  #每个点距离离中位的距离

                    # coords = block[:, :3]
                    colors = np.ascontiguousarray(block[:, 3:6]) / 127.5 - 1 #color归到-1 to 1
                    ...
                    if split != 'test':
                        ...
                        sem_labels = remapper[np.array(sem_labels)]  #可能为了避免不存在的类，规范一下
                        ...
                        uniqueInstances = (np.unique(instance_labels))[1:].astype(np.int32)#其中多少个实例不包括地面
                        remapper_instance = np.ones(50000) * (-100)
                        for i, j in enumerate(uniqueInstances):  #给实例重新排序
                            remapper_instance[j] = i

                        instance_labels = remapper_instance[instance_labels.astype(np.int32)] #重整理instence的值并输回去（数值重整理+去掉不要的）
                        ...
                        if split == 'train' and (len(uniqueInstances) < 10 or
                            ...
                            # sem_labels = sem_labels[:,np.newaxis]  #切割文件为小的txt文件
                            # instance_labels = instance_labels[:,np.newaxis]
                            # with open(outFilePath, "w") as file:
                            #     t = np.concatenate((coords, colors, sem_labels, instance_labels),axis=1)
                            #     for row in t:
...
if __name__ == '__main__':
    data_folder = '/root/autodl-tmp/Stpls3d'
    ...
    #valSplit = [5]
    split =  os.path.join(data_folder, 'val_250m')
