...
class SoftGroup(nn.Module):
    ...
    def forward(self, batch,semantic_only=False, return_loss=False):
        if return_loss:
            return self.forward_train(semantic_only,**batch)
        else:
            return self.forward_test(semantic_only,**batch)

    @cuda_cast
    def forward_train(self,semantic_only, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
    ...
    def instance_loss(self, cls_scores, mask_scores, iou_scores, proposals_idx, proposals_offset,
        ...
        if (torch.max(assigned_gt_inds)<0):
    ...
    @cuda_cast
    def forward_test(self,semantic_only, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
