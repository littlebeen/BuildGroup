...
class SoftGroup(nn.Module):
    ...
    def __init__(self,
        ...
        sam = build_sam_vit_b(checkpoint="./pretrain/sam_vit_b_01ec64.pth").cuda()
        self.predictor = SamPredictor(sam)
        self.trans_conv = nn.ConvTranspose2d(in_channels=256, out_channels=16, kernel_size=16, stride=16, padding=0)
        ...
        self.input_conv = spconv.SparseSequential(
            spconv.SubMConv3d(
                 self.in_channels, channels, kernel_size=3, padding=1, bias=False, indice_key='subm1'))
        ...
        #New
        self.offset_linear_improve = MLP(channels, 3, norm_fn=norm_fn, num_layers=2)
        self.tiny_unet2 = UBlock([channels, 2 * channels], norm_fn, 2, block, indice_key_id=11)
        self.tiny_unet_outputlayer2 = spconv.SparseSequential(norm_fn(channels), nn.ReLU())
        self.input_conv2 = spconv.SparseSequential(
            spconv.SubMConv3d(
                16, channels, kernel_size=3, padding=1, bias=False, indice_key='subm1'))
    ...
    def forward(self, batch, epoch=0,return_loss=False):
        if return_loss:
            return self.forward_train(epoch,**batch)
        else:
            return self.forward_test(epoch,**batch)

    def draw_features(self,x,savename='out.png'):
        plt.axis('off')
        img = x[0, :, :, :]
    ...
    def sam_info(self,image):
        traget_shape=get_preprocess_shape(image.shape[2],image.shape[3],1024)
        #cv2.imwrite('output.png', image[0].permute(1, 2, 0).cpu().numpy())
        image = F.interpolate(image, size=(traget_shape[0],traget_shape[1]), mode='bilinear', align_corners=False)
        feature = self.predictor.set_torch_image(image,traget_shape)
        #self.draw_features(feature)
        return feature

    def get_point_info(self,feature,pre_size, p2i):
        #with torch.no_grad():
            image = F.interpolate(feature, size=pre_size, mode='bilinear', align_corners=False)
            # image = torch.reshape(image, (image.shape[0],image.shape[1],image.shape[2]*image.shape[3]))
            #self.draw_features(image)
            image = image.permute(0,2,3,1)
            point_feature =torch.zeros(p2i.shape[0],image.shape[-1]).cuda()
            slice_inds = torch.arange(0, p2i.shape[0], dtype=torch.long, device=p2i.device)
            point_feature[slice_inds]= image[p2i[slice_inds,0],p2i[slice_inds,1],p2i[slice_inds,2]]
    ...
    def forward_train(self, epoch,batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
        ...
        pre_size=(kwargs['image'].shape[2],kwargs['image'].shape[3])
        feature = self.sam_info(kwargs['image'])
        feature = self.trans_conv(feature)
        point_feature = self.get_point_info(feature, pre_size, kwargs['p2i'])
        feats = point_feature
        ...
        final_pt_offsets=self.point_enhance(output_feats,coords_float,pt_offsets, spatial_shape, batch_size,batch_idxs,True)

        # point wise losses
        point_wise_loss = self.point_wise_loss(semantic_scores, pt_offsets,final_pt_offsets, semantic_labels,
                                               instance_labels, pt_offset_labels, epoch, kwargs['edges'])
        
        losses.update(point_wise_loss)
        if(epoch>40):
            new_pt_offsets=pt_offsets+final_pt_offsets
        else:
            new_pt_offsets=pt_offsets

        # instance losses
        if not self.semantic_only:
            proposals_idx, proposals_offset = self.forward_grouping(semantic_scores, new_pt_offsets,
    ...
    def point_wise_loss(self, semantic_scores, pt_offsets,pt_offsets_imp, semantic_labels, instance_labels,
                        pt_offset_labels, epoch,edges):
        losses = {}
        ...
        if pos_inds.sum() == 0:
            ...
            offset_loss = F.l1_loss(
                pt_offsets[pos_inds]*edges[pos_inds].unsqueeze(1), pt_offset_labels[pos_inds]*edges[pos_inds].unsqueeze(1), reduction='sum') / pos_inds.sum()
            new_pt_offsets=pt_offsets.detach()
            offset_loss2 = F.l1_loss(
                (new_pt_offsets[pos_inds]+pt_offsets_imp[pos_inds])*edges[pos_inds].unsqueeze(1), pt_offset_labels[pos_inds]*edges[pos_inds].unsqueeze(1), reduction='sum') / pos_inds.sum()
        losses['offset_loss'] = offset_loss
        if(epoch>40):
            losses['offset_loss2']=offset_loss2
        return losses
    ...
    def instance_loss(self, cls_scores, mask_scores, iou_scores, proposals_idx, proposals_offset,
        ...
        # compute iou score loss 是真的iou和预测的iou的插值
    ...
    def forward_test(self,epoch, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
                     semantic_labels, instance_labels, spatial_shape, batch_size,
                     scan_ids, **kwargs):
        color_feats = feats
        if self.with_coords:  #test false
            feats = torch.cat((feats, coords_float), 1)
        ...
        voxel_feats = voxelization(feats, p2v_map) #体素化，为了节省空间
        input = spconv.SparseConvTensor(voxel_feats, voxel_coords.int(), spatial_shape, batch_size)
        ...
        final_pt_offsets=self.point_enhance(output_feats,coords_float,pt_offsets, spatial_shape, batch_size,batch_idxs)

        if(epoch>40):
            # with open('tensor2.txt', 'w') as f:
            #     for row in pt_offsets+coords_float:
            #     # 将每一行转换为字符串并写入文件，行与行之间用换行符分隔
            #         f.write(' '.join(map(str, row.tolist())) + '\n')
            pt_offsets =pt_offsets +final_pt_offsets

        if self.test_cfg.x4_split: #似乎是拆分成4部分为了一个特定的数据集
        ...
        if 'semantic' in self.test_cfg.eval_tasks or 'instance' in self.test_cfg.eval_tasks:
            # point_wise_results = self.get_point_wise_results(coords_float, color_feats, #转换了一下形式
        ...
        if not self.semantic_only:
            ...
            if 'instance' in self.test_cfg.eval_tasks:
                ...
                #ret.update(dict(xyz_pres=kwargs['ground_xyzs']))
                #ret.update(dict(conds=kwargs['conds']))
    ...
    @cuda_cast
    def forward_test2(self,epoch, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats, spatial_shape, batch_size,
                     scan_ids, **kwargs):
        color_feats = feats
        if self.with_coords:  #test false
            feats = torch.cat((feats, coords_float), 1)
    ...
    def forward_backbone(self, input, input_map, x4_split=False, lvl_fusion=False):
        if x4_split:
            ...
            if not lvl_fusion:
                ...
                # output_feats2 = torch.cat([output_feats[input_map.long()]],dim=1)

        semantic_scores = self.semantic_linear(output_feats)
        offset_feats = self.offset_encoder(output_feats)
        pt_offsets = self.offset_linear(offset_feats)
        #pt_offsets = torch.clamp(pt_offsets, min=-2, max=2)
        return semantic_scores, pt_offsets, offset_feats

    def point_enhance(self,output_feats,coords_float,pt_offsets, spatial_shape, batch_size,batch_idxs,is_train=False):
        new_feature=output_feats.detach()
        pt_offsets_no_grad=pt_offsets.detach()
        center=coords_float + pt_offsets_no_grad
        center = center-center.min(0)[0]
        new_batch_idxs = batch_idxs.unsqueeze(1)
        center=torch.cat([new_batch_idxs,center],dim=1)

        voxel_coords, v2p_map, p2v_map = voxelization_idx(center.long().cpu(), batch_size)
        voxel_feats = voxelization(new_feature, p2v_map.cuda())
        spatial_shape2 =  center.max(0)[0][1:]
        spatial_shape2[spatial_shape2<1]=1
        spatial_shape2[2]=10
        input = spconv.SparseConvTensor(voxel_feats, voxel_coords.int().cuda(), spatial_shape2, batch_size)
        output = self.input_conv2(input)
        output = self.tiny_unet2(output)
        feats = self.tiny_unet_outputlayer2(output)
        output_feature = feats.features
        offset_feats = self.offset_linear_improve(output_feature)
        offset_feats = offset_feats[v2p_map.long()]
        if not is_train:
            offset_feats = torch.clamp(offset_feats, min=-2, max=2)
        return offset_feats
    ...
    def forward_grouping(self,
        ...
        for class_id in range(self.semantic_classes):
            ...
            object_idxs = (scores > self.grouping_cfg.score_thr).nonzero().view(-1) #超过一定阈值可能为当前class
            if object_idxs.size(0) < self.test_cfg.min_npoint: #这个class的点要够多
                continue
            batch_idxs_ = batch_idxs[object_idxs]
            coords_ = coords_float[object_idxs]
            pt_offsets_ = pt_offsets[object_idxs] #把这个class的点取出来
            if with_pyramid:
            ...
            batch_offsets_ = self.get_batch_offsets(batch_idxs_, batch_size) #没一个batch点的开端
            ...
            proposals_idx, proposals_offset = bfs_cluster(class_numpoint_mean, neighbor_inds.cpu(),
                                                          start_len.cpu(), npoint_thr, class_id) #每个instance的id还有开端点id
            if with_pyramid:
                if level > 1 or not lvl_fusion:
                    proposals_idx, proposals_offset = self.pyramid_inverse_map(
                        proposals_idx, proposals_offset, coords_.size(0), l2p_map)
            proposals_idx[:, 1] = object_idxs[proposals_idx[:, 1].long()].int() #相对的点id换成绝对的点id

            # merge proposals
            if len(proposals_offset_list) > 0:
                proposals_idx[:, 0] += sum([x.size(0) for x in proposals_offset_list]) - 1  #从上一个的最后开始
    ...
    def get_instances(self,
        ...
        for i in range(self.instance_classes):
            if i in self.sem2ins_classes:
                ...
                #has_false = torch.any(mask_inds == False)
                # if(has_false==True):
                #     n=1
                #cur_proposals_idx = proposals_idx[mask_inds].long()
                cur_proposals_idx = proposals_idx.long()
                ...
                #上面那个容易爆显存
                batch_size = 1000  
                npoint = torch.zeros(mask_pred.size(0)).cuda() 

                for i in range(0, mask_pred.size(1), batch_size):
                    batch = mask_pred[:, i:i+batch_size] 
                    npoint += batch.sum(1) 
                inds = npoint >= self.test_cfg.min_npoint
                # has_false = torch.any(inds == False)
                # if(has_false==True):
                #     n=1
                cls_pred = cls_pred[inds]
                score_pred = score_pred[inds]
    ...
    def get_batch_offsets(self, batch_idxs, bs):
        ...
        for i in range(bs): #每个batch取一下
            t = (batch_idxs == i)
            batch_offsets[i + 1] = batch_offsets[i] + (batch_idxs == i).sum()
        assert batch_offsets[-1] == batch_idxs.shape[0]
        return batch_offsets #每个batch的点累加
