...
class Instance(object):
    ...
    def __init__(self, mesh_vert_instances, instance_id,xyz_pres=None):
        if (instance_id == -1):
            return
        ...
        self.volume=0
        if(xyz_pres!=None):
            points = xyz_pres[mesh_vert_instances==instance_id]
            self.volume = get_volume1(points.numpy())
...
def get_instances(ids, class_ids, class_labels, id2label,xyz_pres=None):
    instances = {}
    ...
    for id in instance_ids:
        if id == 0:
            continue
        if(xyz_pres!=None):
            inst = Instance(ids, id,xyz_pres)
            all_volume+=inst.volume
        else:
            inst = Instance(ids, id)
        if inst.label_id in class_ids:
            instances[id2label[inst.label_id]].append(inst.to_dict())
    return instances, all_volume
