...
class ScanNetEval(object):
    ...
    def evaluate_matches(self, matches):
        ...
        vol = np.zeros((len(dist_threshes), len(self.eval_class_labels), len(ious)), float)
            ...
            for oi, iou_th in enumerate(ious):
                ...
                for li, label_name in enumerate(self.eval_class_labels):
                    ...
                    for m in matches:
                        ...
                        vol_match = np.zeros(len(gt_instances))
                        # collect matches
                            ...
                            for pred in gt['matched_pred']:
                                ...
                                if iou > iou_th:
                                    ...
                                    # otherwise set score
                                        ...
                                        vol_match[gti]=pred['volume']  
                            if not found_match:
                                hard_false_negatives += 1
                                all_volume+=gt['volume_gt']
                            else:
                                all_volume+=abs(gt['volume_gt']-vol_match[gti])  
                    ...
                    vol[di, li, oi] = all_volume
        self.all_vol.append(vol[0, 0, 6]/matches['gt_0']['pred']['building'][0]['all_volume'])
        #print(vol[0, 0, 6])
        return ap, rc

    def get_mean_vol(self):
        return sum(self.all_vol) / len(self.all_vol)
    ...
    def assign_instances_for_scan(self, preds, gts, xyz_pres=None):
        """get gt instances, only consider the valid class labels even in class
        agnostic setting."""
        all_volume=0
        if(xyz_pres!=None):
            gt_instances,all_volume = get_instances(gts, self.valid_class_ids, self.valid_class_labels,
                                            self.id2label,xyz_pres)
        else:
            gt_instances,vol = get_instances(gts, self.valid_class_ids, self.valid_class_labels,
                                            self.id2label)
        # associate
        if self.use_label:
        ...
        for pred in preds:
            ...
            if 'volume' in pred:
                pred_instance['volume'] = pred['volume']
            else: 
                pred_instance['volume'] = 0
            pred_instance['all_volume'] = all_volume
    ...
    def evaluate(self, pred_list, gt_list, logger, xyz_pres=None):
        """
        Args:
            pred_list:
                for each scan:
        ...
        if(xyz_pres!=None):
            for i in range(len(pred_list)):
                results.append(self.assign_instances_for_scan(pred_list[i], gt_list[i],xyz_pres))
        else:
            for i in range(len(pred_list)):
                results.append(self.assign_instances_for_scan(pred_list[i], gt_list[i]))
        ...
        #logger.info(ap_scores)
        avgs = self.compute_averages(ap_scores, rc_scores)
        print()
        # print
        if(xyz_pres==None):
            self.print_results(avgs, logger)
        return avgs
